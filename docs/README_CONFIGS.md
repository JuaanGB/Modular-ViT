...
| `cnn`              | Extrae características locales mediante dos convoluciones antes de la proyección a tokens.                                                                                                                                                        |
...
## CNN Patch Embedding (`type: "cnn"`)

Implementación basada en una etapa convolucional previa a la generación de tokens.

### Parámetros

| Parámetro     | Tipo  | Obligatorio | Descripción                                                               |
| ------------- | ----- | ----------- | ------------------------------------------------------------------------- |
| `size`        | `int` | Sí          | Tamaño del parche utilizado en la proyección final.                       |
| `kernel_size` | `int` | No          | Tamaño del kernel de las convoluciones iniciales. Valor por defecto: `3`. |
...
1. La imagen se procesa mediante dos capas convolucionales consecutivas.
2. Cada convolución incorpora:
   - Batch Normalization.
   - Activación GELU.
3. Las características locales obtenidas se proyectan posteriormente al espacio de embeddings mediante una convolución con:
...
4. Los mapas resultantes se transforman en una secuencia de tokens para el Transformer.

### Características

- Introduce sesgos inductivos propios de las CNN.
- Mejora la extracción de patrones locales de bajo nivel.
- Mantiene la estructura global del Vision Transformer.
- Añade un coste computacional reducido respecto al modelo base.
...
| `attention-pooling` | Aprende pesos de importancia para cada token y realiza una suma ponderada. | `hidden_dim` (opcional) |
...
## Attention Pooling (`type: "attention-pooling"`)

Obtiene una representación global aprendiendo automáticamente qué tokens son más relevantes para la tarea.

### Parámetros

| Parámetro    | Tipo  | Obligatorio | Descripción                                                                                                               |
| ------------ | ----- | ----------- | ------------------------------------------------------------------------------------------------------------------------- |
| `hidden_dim` | `int` | No          | Dimensión oculta utilizada por la red MLP que calcula los pesos de atención. Si no se especifica, se utiliza `embed_dim`. |
...
Para cada token final producido por el Transformer:

1. Se calcula una puntuación escalar mediante una pequeña red MLP.
2. Las puntuaciones se normalizan mediante una función softmax.
3. Los pesos obtenidos se utilizan para realizar una suma ponderada de todos los tokens.

### Características

- Generaliza el comportamiento de Global Average Pooling.
- Permite asignar distinta importancia a cada token.
- Introduce un número reducido de parámetros adicionales.
- Puede capturar información global relevante sin depender de un token CLS específico.
